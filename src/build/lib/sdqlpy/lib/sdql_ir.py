...
class TupleType(Type):
    ...
    def __getitem__(self, key):
        if key == 0:
            ...
            raise IndexError("TupleType indices must be 0 or 1")
...
class StreamParam(Expr):
    def __init__(self, param: str):
        super().__init__([])
        self.name: str = "ent." + param

    def printInnerVals(self):
        return " | " + "ent." + str(self.name)
    
class AggSumColumn(Expr):
    def __init__(self, agg: str, param: str):
        self.agg = agg
        self.number = param
    
class MakeTuple(Expr):
    def __init__(self, elems: List[StreamParam]):
        super().__init__([])
        self.elems = elems     
...
class UpdateMapExpr(Expr):
    def __init__(self, map: M3Map, key: MakeTuple, value: Expr):
        self.mapName = map
        self.key = key
        self.value = value

class ForEachUpdateMapExpr(Expr):
    def __init__(self, map: M3Map, key: MakeTuple, value: Expr):
        self.mapName = map
        self.key = key
        self.value = value
...
class GVOD(Expr):
    def __init__(self, map: M3Map, key: MakeTuple):
        self.mapName = map
        self.key = key
...
class AugAssign(Expr):
    def __init__(self, target: Expr, op: str, value: Expr, bodyExpr: Expr):
        self.target = target
        self.op = op + "="
        self.value = value
...
class RegexMatch(Expr):
    def __init__(self, pattern: Expr, matcher: Expr):
...
# def newOpID():
#     if not hasattr(newOpID, "counter"):
#         newOpID.counter = 0
#     newOpID.counter += 1
#     return newOpID.counter

def MakeTupleBuilder(paramList: List[StreamParam]):
    return MakeTuple(paramList)

def UpdateMapBuilder(map: M3Map, key: MakeTuple, value: Expr):
    return UpdateMapExpr(map, key, value)

def ForEachUpdateMapBuilder(map: M3Map, key: MakeTuple, value: Expr):
    return ForEachUpdateMapExpr(map, key, value)

def M3MapBuilder(name: str):
    return M3Map(name)

def GVODBuilder(map: M3Map, key: MakeTuple):
    return GVOD(map, key)
