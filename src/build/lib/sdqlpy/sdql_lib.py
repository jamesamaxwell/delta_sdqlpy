...
def updateMapForEach(map, key, value):
    print("for each update map")
    return

def regex_match(regex, comparator):
    return 0
...
def getAgg(agg, param): # If param is out of agg column range it means the result of the agg
    return (agg, param)

def dictTranspose(dic):
    print(dic)
    return
...
def sdqlpy_init(execution_mode=0, threads_count=1):
    ...
    print("end of sdqlpy_init")

def final_result(in_type):
    reload(site)
...
def agg_sum(in_type, ent_type, return_type):
    reload(site)
    def actual_decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
