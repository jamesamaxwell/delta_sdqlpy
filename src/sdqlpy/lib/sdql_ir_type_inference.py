...
def infer_type(AST: Expr, context: dict, cache: dict):
    ...
    if inputType == ConstantExpr:
    ...
    elif inputType == UpdateMapExpr: # mapName, key, value
        cache[AST.mapName.id] = infer_type(AST.mapName, context, cache)
        cache[AST.key.id] = infer_type(AST.key, context, cache)
        infer_type(AST.value.id, context, cache)
        cache[AST.value.id] = context[AST.mapName.name].toType # infer_type(AST.value, context, cache)
        cache[AST.id] = IntType()
        return cache[AST.id]
        ...
        infer_type(AST.key, context, cache)
        cache[AST.id] = IntType()
        return cache[AST.id]
        ...
        cache[AST.id] = TupleType(tmpList)
        return cache[AST.id]
